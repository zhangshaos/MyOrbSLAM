...
namespace ORB_SLAM3
{
  ...
  // vbPrevMatched[i] means F1.i-th keypoint match F2.KeyPoint.pt
  int ORBmatcher::SearchForInitialization(Frame& F1, Frame& F2, vector<cv::Point2f>& vbPrevMatched, vector<int>& vnMatches12, int windowSize)
  {
    ...
    for (size_t i1 = 0, iend1 = F1.mvKeysUn.size(); i1 < iend1; i1++)
    {
      ...
      // 在 F2 中搜索与 F1 中特征点 i1 距离近的待匹配特征点
      ...
      for (vector<size_t>::iterator vit = vIndices2.begin(); vit != vIndices2.end(); vit++)
      {
        ...
        if (vMatchedDistance[i2] <= dist) // 寻找描述符最近的特征点匹配
          continue;
      ...
      } // bestIdx2 表示和 i1 最近的特征点索引，bestDist、bestDist2 分别为和 i1 最近的两个特征点距离
  ...
  int ORBmatcher::SearchForMatches(Frame& F1, Frame& F2, std::vector<int>& vnMatches12, int windowSize)
  {
    int nmatches = 0;
    ...
    for (size_t i1 = 0, iend1 = F1.mvKeys.size(); i1 < iend1; i1++)
    {
      ...
      int level1 = kp1.octave;
      // ignore other scale diff from 0.
      if (level1 > 0)
        continue;

      // 在 F2 中搜索与 F1 中特征点 i1 距离近的待匹配特征点
      vector<size_t> vIndices2 = F2.GetFeaturesInArea(F1.mvKeys[i1].pt.x, F1.mvKeys[i1].pt.y, windowSize, level1, level1);
