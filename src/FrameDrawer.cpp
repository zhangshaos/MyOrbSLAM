...
cv::Mat FrameDrawer::DrawFrame(bool bOldFeatures) {
  ...
  else if (state == Tracking::OK && bOldFeatures)  // TRACKING GOTO HERE
  {
    // Draw rects first
    for (auto& r : rects) {
      cv::rectangle(im, r, cv::Scalar(0., 0., 255.));
    }
    mnTracked = 0;
    mnTrackedVO = 0;
    const float r = 5;
  ...
  cv::Mat imWithInfo;
  DrawTextInfo(track_id, im, state, imWithInfo);
