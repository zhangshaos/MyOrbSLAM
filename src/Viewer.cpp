...
void Viewer::Run()
{
    ...
    while(1)
    {
        ...
        mpMapDrawer->DrawCurrentCamera(Twc); // 摄像头
        if(menuShowKeyFrames || menuShowGraph || menuShowInertialGraph)
            mpMapDrawer->DrawKeyFrames(menuShowKeyFrames,menuShowGraph, menuShowInertialGraph); // 关键帧
        if(menuShowPoints)
            mpMapDrawer->DrawMapPoints(); // 点云
        // 上面的都是在 Map Viewer 窗口中绘图
        pangolin::FinishFrame();

        cv::Mat toShow;
        cv::Mat im = mpFrameDrawer->DrawFrame(true);
        // 在 Current Frame 窗口中绘画
